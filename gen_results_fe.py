...
def rgen_fe(X, y, base_model, params_ensamble):
  param_grid_fe = {'window': range(1, 9), 'past_end': range(1, 9)}
  ...
  results.to_csv(f'results/regression_fe_ridge2_full.csv')
  rs_mean.to_csv(f'results/regression_fe_ridge2_mean.csv')
...
def main():
  ...
  params_ensemble = {'th_mode': 'daytime', 'th_multiplier': 1.00, 'th_majority': 0.1, 'mk_size': 3}
  #rgen_fe(X, y, MLPRegressor(hidden_layer_sizes=(7, 6), learning_rate='adaptive', activation='relu'), params_ensemble)
  rgen_fe(X, y, Ridge(alpha=0.333), params_ensemble)
