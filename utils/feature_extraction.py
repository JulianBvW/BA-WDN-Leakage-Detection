...
def past_days_transform(X, nodes=['10', '11', '12', '13', '21', '22', '23', '31', '32'], past_start=1, past_end=2, hours_per_day=48):
  ...
  for x in X:
    ...
    for past_day in range(past_start, past_end):
      ...
      fillers = [x.loc[:hours_per_day-1, nodes].add_suffix(f'_past{past_day}') for i in range(past_day)]

      # Then add the shifted days
      shifted = [x.loc[:len(x)-hours_per_day*past_day-1, nodes].add_suffix(f'_past{past_day}')]
