...
def print_metrics(y_true_list, y_pred_list):
  ...
  print(f'Detection Time      \tWie viele Stunden dauerte es bis zum erkennen?')
  print(f' -> Mean: {round(detection_time_mean(y_true_list, y_pred_list), 3)}h\tStd: {round(detection_time_std(y_true_list, y_pred_list), 3)}h\tMedian: {round(detection_time_median(y_true_list, y_pred_list), 3)}h')
